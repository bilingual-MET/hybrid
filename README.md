...
7. Run the [`collect_and_clean_tweets.py`](https://github.com/bilingual-MET/hybrid/blob/main/preprocessing/collect_and_clean_tweets.py) script that you moved to the `nga-kupu-master/scripts` folder. This script gathers tweets from the past week from a predefined list of users ([`users.csv`](https://github.com/bilingual-MET/hybrid/blob/main/preprocessing/users.csv)), then cleans the tweets and generates the RMT labels that are needed as input to the hybrid architecture (below).
